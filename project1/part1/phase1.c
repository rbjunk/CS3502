...
void* teller_thread(void* arg) //teller function, which will be sent to each thread as an argument
{
    ...
    unsigned int seed = time(NULL) + teller_id; //initialize a (somewhat) unique seed for this thread
...
int main(int argc, char *argv[])
{
    ...
    for (int i = 0; i < CURRENT_ACCOUNT_AMOUNT; i++) //initialize the accounts with a starting balance
    {
